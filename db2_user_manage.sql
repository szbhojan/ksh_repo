...
db2look -d wcq01 -z wscomusr -e -o DEFAULT_SHIPPING_ADDRESS_SP

db2 "SELECT PROCNAME, TEXT FROM SYSCAT.PROCEDURES WHERE PROCSCHEMA = 'WSCOMMUSR'";

db2 "SELECT PROCNAME, TEXT FROM SYSCAT.PROCEDURES WHERE PROCNAME like 'DEFAULT_SHI%'";

------------------------------------------------------------------------------------

db2 " SELECT AUTHID FROM SYSIBMADM.AUTHORIZATIONIDS WHERE AUTHIDTYPE = 'U' \
AND EXISTS ( SELECT 1 FROM TABLE(SYSPROC.AUTH_LIST_ROLES_FOR_AUTHID(AUTHID, 'U')) AS ROLES \
...
------------------------------------------------------------------------------
db2 "SELECT AUTHID FROM SYSIBMADM.AUTHORIZATIONIDS WHERE AUTHIDTYPE = 'U'";
db2 "SELECT AUTHID FROM SYSIBMADM.AUTHORIZATIONIDS WHERE AUTHIDTYPE = 'G'";
db2 "SELECT AUTHID FROM SYSIBMADM.AUTHORIZATIONIDS WHERE AUTHIDTYPE = 'R'";

db2 " SELECT AUTHID FROM SYSIBMADM.AUTHORIZATIONIDS WHERE AUTHIDTYPE = 'U' AND EXISTS ( SELECT 1 FROM TABLE(SYSPROC.AUTH_LIST_ROLES_FOR_AUTHID(AUTHID, 'U')) AS ROLES WHERE ROLES.ROLENAME = 'PHANTOM_UPD') " ;

query that lists all users and the roles they belong to:
--------------------------------------------------------
db2 " SELECT A.AUTHID AS USERNAME, R.ROLENAME FROM SYSIBMADM.AUTHORIZATIONIDS A \
JOIN TABLE(SYSPROC.AUTH_LIST_ROLES_FOR_AUTHID(A.AUTHID, 'U')) R ON 1=1 \
WHERE A.AUTHIDTYPE = 'U' \
ORDER BY R.ROLENAME, A.AUTHID " ;
------------------------------------------------------------
...
will return all roles granted to the specified authorization ID, including the grantor, grantee, and whether the grant includes admin option:
-------------------------------------------------------------------------------------------------------------------------------------------------
db2 "SELECT GRANTOR, GRANTORTYPE, GRANTEE, GRANTEETYPE, ROLENAME, CREATE_TIME, ADMIN FROM TABLE (SYSPROC.AUTH_LIST_ROLES_FOR_AUTHID('SINHAA', 'U')) AS T" ;
